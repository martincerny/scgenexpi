...
regulator_expression_mean = matrix(abs(rnorm(num_cells * num_regulators, 0, regulator_prior_sigma)), nrow = num_cells, ncol = num_regulators);
...
fit = stan('overall_steady.stan', data = observed)
summary(fit, pars = "w")$summary[,c("n_eff","Rhat")]
evaluate_single_param(extract(fit), "w", true$w)
evaluation_summary(extract(fit), true, printParamsResults = FALSE)
